...
VARIANCE_VALUES = [5, 10, 20, 40, 100, 120, 150, 200, 250]
N_KEYPOINTS = 100
...
# print(np.linalg.norm(descriptors - descriptors))

for variance in VARIANCE_VALUES:
    noisy_img = add_noise(origin_img, variance)
    filename = 'img_with_noise_{}.jpg'.format(variance)

    noisy_kps, noisy_descriptors = extract_features(noisy_img, N_KEYPOINTS)

    noisy_img_with_kps = cv2.drawKeypoints(noisy_img, noisy_kps, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
    ...
    # img3 = cv2.drawMatches(origin_img, kps, origin_img, noisy_kps, matches, None, flags=2)
    # utils.save_image(output_img_dir, img3, 'result.jpg', 100)
    ...
    print('Found {} kps in noisy img with variance = {}. Matches = {}'.format(len(noisy_kps), variance, len(matches)))